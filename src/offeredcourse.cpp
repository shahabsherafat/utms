...
void OfferedCourse::write_general_info(vector<string>& output){
    output.push_back(to_string(id) + SPACE + name + to_string(capacity) + SPACE + professor_name + NEWLINE);
}

void OfferedCourse::write_detailed_info(vector<string>& output){
    output.push_back(to_string(id) + SPACE + name + to_string(capacity) + SPACE + professor_name 
